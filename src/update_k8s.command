...
tar czvf master.tgz *
cp -f master.tgz ~/coreos-k8s-solo/kube/
# clean up tmp folder
rm -rf ~/coreos-k8s-solo/tmp/*
echo " "
...
~/coreos-k8s-solo/bin/wget -N -P ~/coreos-k8s-solo/tmp https://storage.googleapis.com/kubernetes-release/release/$K8S_VERSION/bin/linux/amd64/kubectl
tar czvf nodes.tgz *
cp -f nodes.tgz ~/coreos-k8s-solo/workers/
# clean up tmp folder
rm -rf ~/coreos-k8s-solo/tmp/*
...
vagrant ssh k8solo-01 -c "sudo /usr/bin/mkdir -p /opt/bin && sudo tar xzf /home/core/master.tgz -C /opt/bin && sudo chmod 755 /opt/bin/* "
