...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    NSString *home_folder = [NSHomeDirectory() stringByAppendingPathComponent:@"coreos-k8s-solo"];
    
    BOOL isDir;
    if([[NSFileManager defaultManager] fileExistsAtPath:home_folder isDirectory:&isDir] && isDir)
    // if coreos-k8s-solo folder exists
    {
        // set resouces_path
        ...
        [[NSFileManager defaultManager] createFileAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"coreos-k8s-solo/.env/resouces_path"]
        ...
        [[NSFileManager defaultManager] createFileAtPath:[NSHomeDirectory() stringByAppendingPathComponent:@"coreos-k8s-solo/.env/version"]
    ...
    else
    {
        ...
        [alert setMessageText:@"CoreOS+Kubernetes Solo was not set."];
...
- (IBAction)Start:(id)sender {
        ...
        fileExistsAtPath:home_folder isDirectory:&isDir] && isDir)
    {
        ...
        notification.title = @"CoreOS+Kubernetes Solo will be up shortly";
...
- (IBAction)Pause:(id)sender {
    // send a notification on to the screen
    NSUserNotification *notification = [[NSUserNotification alloc] init];
    notification.informativeText = @"CoreOS+Kubernetes Solo will be suspended";
    [[NSUserNotificationCenter defaultUserNotificationCenter] deliverNotification:notification];
...
- (IBAction)Stop:(id)sender {
    // send a notification on to the screen
    NSUserNotification *notification = [[NSUserNotification alloc] init];
    notification.informativeText = @"CoreOS+Kubernetes Solo will be stopped";
    [[NSUserNotificationCenter defaultUserNotificationCenter] deliverNotification:notification];
...
- (IBAction)Restart:(id)sender {
    // send a notification on to the screen
    NSUserNotification *notification = [[NSUserNotification alloc] init];
    notification.informativeText = @"CoreOS+Kubernetes Solo will be reloaded";
    [[NSUserNotificationCenter defaultUserNotificationCenter] deliverNotification:notification];
...
- (IBAction)destroy:(id)sender {
    // send a notification on to the screen
    NSUserNotification *notification = [[NSUserNotification alloc] init];
    notification.informativeText = @"CoreOS+Kubernetes Solo will be destroyed";
    [[NSUserNotificationCenter defaultUserNotificationCenter] deliverNotification:notification];
...
- (IBAction)initialInstall:(id)sender
{
    ...
    if([[NSFileManager defaultManager]
        ...
        [self displayWithMessage:@"CoreOS+Kubernetes Solo" infoText:msg];
    }
    else
    {
...
//    NSString *app_version = [NSString stringWithFormat:@"%@.%@", version, build];
    ...
    NSString *mText = [NSString stringWithFormat:@"%@ %@", @"CoreOS+Kubernetes Solo for OS X", app_version];
    NSString *infoText = @"It is a simple wrapper around the CoreOS-Vagrant, which allows to control CoreOS+Kubernetes Solo via Status Bar !!!";
